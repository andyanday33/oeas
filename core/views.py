...
def question_answer_view(request, exam_pk, question_pk):
    ...
    for i, eq in enumerate(exam_questions):
        if eq == question:
            ...
            if i >= 1:
                previous_question = exam_questions[i-1]
            break
    ...
    if previous_question is not None:
        try:
            previous_url = reverse('question_answer', args=[exam_pk, previous_question.pk])
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if timezone.now() > exam.end_date:
                messages.success(request, "Exam's time is over. Your choice is not saved")
                return HttpResponseRedirect(reverse('student_exam_detail', args=[exam.pk]))
...
def student_score_calculate(student_pk, exam_pk):
    # calculates s
    exam_questions = Question.objects.filter(exam=exam_pk)
    students_answers = AnsweredQuestion.objects.filter(student = student_pk,question__in= exam_questions)

    try:
        if students_answers: # cevapları var
            has_answered = True
            exam_correctness = []

            for sa in students_answers:
                try:
                    exam_correctness.append(sa.answer_choice.is_true_choice)
                except:
                    exam_correctness.append(False)
            score = sum(exam_correctness)/len(exam_questions)*100
...
def add_student_to_exam_view(request, exam_pk):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            messages.warning(request, 'You can not add no one.')
            sea = StudentExamAccess.objects.filter(exam=exam)
            return render(request, 'core/add_student_to_exam.html', context)
...
def score_list_view(request,exam_pk):
    exam = Exam.objects.get(pk=exam_pk)
    context= {"exam":exam}

    exams_students = StudentExamAccess.objects.filter(exam__pk=exam_pk)

    students_and_scores = []

    for es in exams_students:
        score = student_score_calculate(es.student.pk,es.exam.pk)
    ...
    context['students_and_scores'] = students_and_scores

    
    return render(request, 'core/instructor_exam_results.html', context)
